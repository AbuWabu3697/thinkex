...
- **Auth without Google OAuth**: when Google creds are unset, the auth page automatically shows a **Continue as guest** button (anonymous session). For headless automation, hit the endpoint directly: `POST /api/auth/sign-in/anonymous` with header `Content-Type: application/json` and body `{}`, then reuse the cookie; this is enough to reach `/home` and create a workspace.
